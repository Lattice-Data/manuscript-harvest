...
Every example above assumes a whole text and an empty `text_defects` — no count is given here on purpose, because the last one went stale. Add a verified defect in the main source, or a `no_methods_content` from anywhere, and examples 2, 5 and 7 are capped at "unclear" by Stage B rather than resolving to "no". A defect in a supplementary source alone does not cap them: see Stage B.
...
- Stage A result **"not_applicable"** → unchanged. The cap exists because missing text can hide a pairing sentence; a review has no pairing to hide, and a review very often reports `text_completeness` = "methods_missing" precisely because it has no Methods. Capping it would turn every review into an `unclear` for triage to re-read.
...
**Why the scope is what it is.** The cap withholds a negative only where the missing text could have carried the pairing sentence. A garbled table in a supplementary reporting summary could not, so it does not cap — it is still recorded, and a curator still sees it. An absent methods section could, wherever it lives, so that one caps from any source. (Curator decision, 2026-09-16.)
...
| Degraded or truncated text that would otherwise resolve to "no" | Stage B caps it at "unclear" with `unresolved_reason = degraded_text`, on a harness-verified defect in the main source or a missing methods section anywhere | Disable Stage B and keep the "no", filtering on `text_defects` downstream |
| A defect in a supplementary source only | Recorded in `text_defects`, does NOT cap — it could not have hidden a pairing sentence | Widen `decide.yaml: cap.when_defect_in_sources` to every source |
