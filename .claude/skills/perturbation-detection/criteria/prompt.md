...
| Constant | Value |
|---|---|
| `task_version` | `{{TASK_VERSION}}` — ONE number for this pack, replacing the old `prompt_version` + `schema_version` pair. Declared in `task/task.yaml` and spliced in by `harness.prepare`, so no literal appears in this file and it cannot go stale. Bump it for any change to what a record should contain or how it is judged. |
...
Fill `{{PAPER_ID}}`, `{{PAPER_TEXT}}` and `{{ASSEMBLY}}` for one paper, call the model once, verify the quotes, recompute the determination, then write one JSONL record. The model returns exactly one JSON object.

`{{ASSEMBLY}}` is what the assembly below REMOVED, stated to the model because Step 0 asks it whether the text is complete. Its contents are specified in **Batch wrapper spec, step 2**; the short version is that every fact in it is derived from the assembly that just ran, so it cannot drift from what was actually done.
...
**Read `ASSEMBLY:` before you judge the text.** The text you were given is not the published article — a pipeline assembled it, and removed things on the way. `ASSEMBLY:` states what it removed for THIS paper: which sources were supplied and how large they are, how many supplementary files were supplied of how many were found, which sections were stripped from every source, which kinds of content reached you at all, and whether a length budget forced further cuts. It is the pipeline talking about itself, not part of the paper: never quote from it.

**So "full" means: nothing is missing BEYOND what `ASSEMBLY:` says was removed.** These are the pipeline's own cuts, and none of them is `truncated`, `methods_missing` or `partial`:
- the text ending on a bare heading with nothing under it — "Associated Data", "Supplementary Materials", "Footnotes", "Contributor Information" — because what was under it is on the removed list;
- an absent reference list, or one that stops after a single entry;
- a sentence cut mid-clause at the very END of a source, including a preprint licence footer (`"...has granted medRxiv a license to display the preprint in perpetuity. It is made"`);
- a table, a figure image, or a supplementary file that `ASSEMBLY:` does not list as supplied;
- a missing Discussion or Introduction when `ASSEMBLY:` reports a budget truncation that dropped them.

**"full" is the normal answer — 372 of the 392 papers in this corpus are "full".** Report a defect only where you can point at one INSIDE the text you were given: a run of garbled characters, a body that stops mid-Results or mid-Methods, an obviously abstract-only extraction, no methods content anywhere. A cut the pipeline announced is not a defect in the article, and this is asymmetric for a reason — reading the pipeline's own cut as an incomplete paper triggers the damaged text downgrade on a negative that was drawn from complete text.

Set `processing_status`:
- **"ok"** — coherent, readable text of an apparently complete article.
- **"partial"** — readable but visibly incomplete or degraded: large runs of garbled characters, sections that end mid-sentence *within* the article, an obvious front-matter-only or abstract-only extraction, or a body with no Methods-like content. **Held to the same standard as "full" above:** a defect you can point at, not a cut `ASSEMBLY:` announced.
...
Set `text_completeness`: **"full"**, **"truncated"** (text ends abruptly *within* the article, or is explicitly marked as cut), **"methods_missing"** (readable article body with no methods/experimental-procedures content anywhere, including in supplementary sources), or **"unknown"**.

### Step 0a: point at any defect you found

`text_defects` is a required array. **`[]` is the normal, correct and common answer: 20 of the 24 papers in the last acceptance set and 372 of the 392 in this corpus have no defect at all.** It is not a quota, and an empty array is not a failure to look.

Decide the text's integrity FIRST, on what you read, and then record why. This array justifies a judgment you have already made; it never creates one. One entry per defect, shaped `{"source_id": "supp2", "kind": "garbled_run", "quote": "the exact text where you saw it"}`:

- **`kind`** is one of **`ends_mid_sentence`** (a source stops in the middle of a sentence or a section, *anywhere* — it does not have to be at the very end of what you were given), **`explicit_cut_marker`** (the text says it was truncated), **`garbled_run`** (a run of mojibake, control characters or non-linguistic noise in place of content), **`no_methods_content`** (no methods or experimental-procedures text anywhere in any source).
- **`source_id`** is the `<<<SOURCE>>>` block the defect is IN. Per source, not per paper: *"the reporting summary is mojibake and the article is whole"* is two different facts about two sources, and this is how you say it. Use the id you actually read it from.
- **`quote`** is copied verbatim from that source, and is checked against it the same way every other quote in this schema is. **A defect whose quote cannot be found is discarded and does not constrain the determination** — so quote the text you saw rather than describing it.
- **`no_methods_content` is the one kind that takes `"quote": null`**, because an absence has no text to copy. Nothing else may omit the quote.

**None of the following is a defect.** Each is a cut the pipeline made on purpose, and `ASSEMBLY:` says which ones apply here:

- the text ending on a bare heading with nothing under it — "Associated Data", "Supplementary Materials", "Footnotes";
- an absent or one-line reference list;
- a missing table, a missing figure image, or a supplementary file `ASSEMBLY:` does not list as supplied;
- a missing Discussion or Introduction where `ASSEMBLY:` reports a budget truncation — **but the budget truncation ITSELF is a defect: report it as `explicit_cut_marker` on the main source, quoting `ASSEMBLY:`'s own truncation line.** The pipeline cut real content to fit a budget, and that is the one announced cut that still leaves the article incomplete;
- a review or commentary having no Methods. That is its article type (Step 0b), not a defective extraction.
...
### Step 0b: does this paper report a study of its OWN?

Set `reports_primary_research`:
- **"yes"** — the paper reports work its authors did: it has Methods describing what they performed, or results presented as their own.
- **"no"** — the paper reports no study of its own. A review, a commentary, a perspective, an editorial, a meeting report, a news-and-views.

If `reports_primary_research` = "no", **return immediately**, exactly as a failed extraction does: `perturbation_present` = "not_applicable", `unresolved_reason` = "none", empty `perturbations`, `samples` and `suppressed_candidates`, and the article type named in `ambiguities`. Do not run Steps 1-3.

`not_applicable` is neither a negative nor a gap. A review often describes perturbation experiments at length — `10.1016/j.coi.2022.102188` yielded six of them and twelve sample groups before this gate existed, every one harvested from work its authors did not do. Reporting "no" would assert the opposite of what the text shows; reporting "unclear" would claim the text is deficient when it is complete and simply belongs to another article type. And those six perturbations were not this paper's to report, which is why the gate returns rather than carrying them forward.

**This gate fires rarely. Nearly every paper you will see is primary research, and "yes" is the overwhelmingly common answer.** It is a question about ARTICLE TYPE and nothing else. In particular it is NOT reached by any of these:
- **A paper that reanalyses public or previously published data is primary research.** Re-analysis, harmonization and integration atlases are studies their authors performed. `10.1038/s41467-021-25125-1` builds an atlas almost entirely out of six other groups' datasets and is primary research.
- **A paper whose single-cell data came from a collaborator or a consortium** is primary research.
- **A methods, protocol or benchmarking paper that generated data** is primary research.
- **A paper with no Methods SECTION is not thereby a review.** `text_completeness` = "methods_missing" is a statement about the text you were given; this gate is a statement about the article. Judge the article type from what the article is, not from what the extraction dropped.

**Decide this on article type BEFORE you look at any experiment, and never the other way round.** Do not conclude "no" because the experiments you found look like someone else's; that reasoning would gate the integration atlas above. The gate never suppresses a primary paper's perturbations — if `reports_primary_research` = "yes", this step changes nothing about Steps 1-3.
...
| `disease_model_establishment` | a manipulation used to OBTAIN the material the paper then characterises, rather than one whose effect the paper characterises — a germline disease genotype the animals were born with (nothing applied), an engraftment, or an induction protocol whose resulting tissue rather than whose effect is the subject | **No** — not by the paper's interest in it, and not by comparing two ways of making the model. What promotes an applied manipulation is the paper attributing its findings TO it: studying the response to a diet or an injury is a perturbation |
| `observational_disease_state` | a naturally occurring disease state or donor genotype, with no bench manipulation, **including an infection the subjects acquired naturally** | **Yes**, but by ONE route only: an acquired EXPOSURE whose response is the paper's own question (curator batch 2026-09-15). A genotype or a disease state the donor simply has stays excluded — nothing was applied, so there is nothing to promote. A naturally acquired infection is different in kind: it is an exposure the body reacted to, and where the paper's stated question is that reaction it is reported, per the second half of the EXPOSURE bullet. Being one donor attribute among several is not a question |
...
| `incidental_clinical_therapy` | a therapy applied to patients that the paper treats as its SETTING rather than its VARIABLE, and treatment recorded only as incidental cohort metadata — including a medication the donor was already taking, which is an exposure that occurred outside the study | **Yes**, and this is the one rule whose promotion test is the paper's own treatment of the item: THE GOVERNING QUESTION below. **Read its first sharpening before promoting anything here** — grouping the sequenced samples by the therapy is NOT what promotes it, and that is the mistake this rule actually attracts (curator batch 2026-09-15) |
...
### THE GOVERNING QUESTION, and it settles every boundary rule below

Everything in Step 2 reduces to one question about the thing that was applied:

> **Is the applied thing what the paper is trying to LEARN ABOUT — or is it how the paper OBTAINED the material it then studies?**

Learning about it -> **perturbation**, report it. Obtaining material with it -> the **model** or the **setting**; record it in `suppressed_candidates` under whichever reason names the kind of thing it was. The reasons are LABELS for this one answer, not separate tests, and where a rule below states its own test that test is an illustration of this question rather than a replacement for it.

**When the question above does not settle it, the KIND of manipulation does. Two kinds, and they answer differently (curator rulings 2026-09-06 and 2026-09-07).**

- **EXPOSURE — an external factor the material was subjected to and REACTS to.** Diet, temperature, light, hypoxia, irradiation, a drug or toxin, mechanical trauma or a surgical insult, **and a pathogen: infection is exposure, because "the mice react to the pathogen"**. An exposure is a **PERTURBATION** even when the paper is named after a cell population rather than after the exposure — the curator's ground being that these are "external factors that we could study the mice reaction to". Ruling 9: a western diet fed to induce NASH, in a paper about hepatic macrophage niches. Ruling 10: a T9 contusion, in an injury atlas.

  **WHO applied it decides which test the exposure gets, and this is the one place the answer differs (curator batch 2026-09-15).** The paragraph above is about an exposure the INVESTIGATORS applied as part of the study: diet fed, contusion delivered, virus inoculated, temperature set. For those, the paper's interest is irrelevant — they are perturbations, and the only further question is whether they carry a contrast.

  An exposure that **occurred outside the study** is different. A naturally acquired infection, a medication the patient was already taking, an environmental or dietary history: nobody in the study applied it, and the investigators only recorded what had already happened. Such an exposure is a perturbation **only where the paper's stated question is the response to it**. Where it is one donor attribute among several, it is the setting — record it under `observational_disease_state` for a state nobody applied, or `incidental_clinical_therapy` for a therapy somebody did.

  Both halves are needed, and each was reached by a curator. Ruling 9 is why an investigator-applied exposure does not need to be the paper's subject. The 2026-09-15 batch is why a natural one does: a COVID paper whose question is *how exposure to SARS-CoV-2 affects nasal immune and epithelial cells* reports the infection, while a lung atlas that happens to include COVID-19 donors alongside IPF and healthy ones does not. Both sequence naturally infected people; only the first is asking what the infection did.
- **CONSTRUCTION — a modification that BUILDS the material or the state you wanted.** Engraftment or transplantation of material that becomes the tissue under study, a germline or induced genotype the animal carries, a derivation or differentiation formulation, timed mating to obtain pregnancy. A construction is the **MODEL** — *"in vivo modification that simply gets the mice to tumors"*, closer to a buffer or a suspension concentration than to a treatment. Rulings 7, 11, 12, 13 and 5.

**An exposure needs a CONTRAST to be a variable.** An exposure given to every arm alike is a constant of the protocol, not a perturbation — there is nothing being compared with respect to it. Ruling 8's paper is the case: *"Lrig1CreERT2/+;Apc2lox14/2lox14 were injected with 0.01mM 4-hydroxytamoxifen… and were administered 2.5% DSS in drinking water for the following 6 days. **Control mice received PBS injections followed by DSS.**"* Both arms get the DSS, so the DSS is how every animal was brought to a damaged colon and the contrast is carried entirely by the alleles. Report an exposure only where some sequenced material had it and some did not, or where its dose or duration was varied across sequenced arms. This is NOT the v0.0.9 uniformity precondition that ruling 1 removed: that one asked whether a CONSTRUCTION was uniform, and a construction stays the model whether uniform or not. This asks only whether an exposure has a comparison at all.

**The line between the two kinds: does the applied thing BECOME the material, or does the material REACT to it?** Engrafted AKPS organoids become the tumour whose microenvironment is sequenced — construction. An intranasally inoculated virus is reacted to by the animal's own cells, which are what gets sequenced — exposure. That is why a pathogen and a tumour graft, both living things introduced into a mouse, land on opposite sides.

**Mechanism carries no weight within a kind.** How a strain came to exist — CRISPR, Cre-lox, germline breeding, purchase — is irrelevant: a CRISPANT line and a conditional floxed allele are the same fact about the animal (curator ruling 2026-09-06). What decides for a construction is only whether the question above already promoted it.

**This is a DEFAULT, not a competing test.** The question above always wins where it applies: `Blg-Cre;Brca1f/f;p53+/−` is a construction and still a perturbation, because that paper's stated gap is *"little is known about how BRCA1 loss-of-function… affect the luminal progenitor cell state"* and its finding is *"perturbing Brca1/p53… induces aberrant alveolar differentiation"* (ruling 14). Contrast the constructions that stay models: the engraftment sits in a paper about neutrophils in colorectal cancer, `Col7a1−/−` in a paper about how inflammation predisposes a microenvironment, `UMOD-C125R` in a paper validating a spatial method, `Rag1−/−` in a human thymus atlas. In each the paper says what it does not know, and it is not the lesion.

**Three sharpenings, each of which cost a curator ruling to establish. Every one of them is a case where the intuitive reading is wrong.**

1. **Being the axis you GROUP the data along is not being the SUBJECT.** A paper can organise its samples by a manipulation without that manipulation being what it concludes about. `10.1038/s41467-021-21783-3` takes tissue at gestation days 4.5, 9.5 and 14.5 against nulliparous controls — and concludes that *"unlike alveolar differentiation during gestation"* the `Brca1/p53` effect is cell-autonomous. Gestation is the yardstick; `Brca1` loss is the subject. Ask what the paper CONCLUDES, never what it groups by. This is the same error as reading two treatment timepoints as a treatment study (`10.1038/s41467-025-65049-8`).

   **And WEIGH the mention, because a grouping label can be almost invisible (curator batch 2026-09-15).** `10.1038/s41467-024-55440-2` groups 3 of its 10 sequenced donors by progestin contraception and remarks in passing that "progesterone may be important" for a cell state — one mention in the body, a footnote marker in the donor table, one supplementary panel, against headline axes of menopause and cycle phase. That is a cohort attribute the authors recorded, not a question they asked. Where the applied thing appears ONLY in cohort description and a single supplementary comparison, it is the setting, and an incidental attribution in one clause does not promote it. The reverse also holds: what makes something the subject is the paper saying so where papers say such things — the title, the abstract, the stated gap, the conclusions.
2. **The shape of the comparison decides nothing.** "Diseased against healthy" describes a western-diet NASH paper (perturbation) and an engrafted-tumour paper (model) alike. "A different cell identity against its precursor" describes a paper studying a differentiation cocktail (perturbation) and one merely using it (model) alike. Two earlier versions of these rules keyed on exactly such shapes and both were refuted by the first papers read under them.
3. **Two manipulations in one paper get answered separately, and one can be the model while the other is a perturbation.** `Trem2−/−` on the `5XFAD` Alzheimer's background: `5XFAD` is the model, `Trem2−/−` is a perturbation, because the paper attributes the loss of GPNMB expansion to `Trem2` and compares it against a `Trem2+/+` counterpart at a fixed `5XFAD` state. Do not let a suppressed model absorb everything else done to the same animals.
...
- **A manipulation used to give the samples the disease state the paper studies is the MODEL.** This is THE GOVERNING QUESTION above applied to a disease: obtaining diseased material -> `rule: disease_model_establishment`; studying what the manipulation did to it -> a perturbation. Do not re-derive a test here; the question and its three sharpenings are stated once, above.
  - **A normal physiological state the investigators set up goes through THE GOVERNING QUESTION like everything else — "physiological" is not an exclusion in itself, and reading it as one is the blanket-clause error this prompt removed at v0.0.14.** Pregnancy by timed mating in `10.1038/s41467-021-21783-3` is the model because that paper concludes about `Brca1/p53` and uses gestation only as its yardstick (curator ruling 2026-09-03: *"timed mating to induce pregnancy is not perturbation"*). But a physiological manipulation whose effect the paper REPORTS is a perturbation: caesarean delivery against vaginally-born littermates, in a paper reporting the transcriptional shift across the first hour of extrauterine life, is applied and attributed, so it is reported. Where the answer is the model, use `disease_model_establishment` and say in `why` that the state is physiological rather than diseased — the closed set is deliberately not widened for a single case, and this prompt already tells you to take the closest value and explain.
  - **The route does not matter.** One of those papers engrafted CRISPR-engineered AKPS organoids; the other injected 4-hydroxytamoxifen through the colonoscope to activate Cre in resident cells, against a PBS-injected control. Both are the model. A rule keyed on "cells were introduced" would have caught the first and missed the second — and note that Test 1 does not reach either of them, because something WAS applied in both; they are models by Test 2.
  - **Nor does comparing two ways of making it.** Two cells of origin, two genotypes, or two induction protocols, all sequenced, is still a contrast between models — the same move ruling 1 made for two differentiation media. This is the identity/state analogue: producing a different disease STATE is the model; varying something while the state is held fixed is a perturbation.
  - **What survives as a perturbation** is anything applied to the ESTABLISHED model and compared at fixed disease state: a drug given to tumour-bearing mice against vehicle, a depleting antibody against isotype control, a cytokine against untreated. Those are perturbations and are reported normally — this rule takes nothing away from them.
  - A generic unmodified line (HeLa, HEK293) with no manipulation is not a perturbation and needs no entry. [TOGGLE: count disease-model establishment as a perturbation if curation wants engineered and induced models flagged.]
...
- **A model system's derivation or maintenance formulation is the MODEL when the paper uses it to reach a cell state and then studies that state — and a PERTURBATION when the formulation itself is what the paper is trying to learn about.** Curator clarification, 2026-09-06: *"IF the goal of the research is to study the differentiation cocktail - IT WILL BE a PERTURBATION, if it just means to get to some cell state/fate - it is NOT."* So apply THE GOVERNING QUESTION above; the identity-versus-factor contrast below is an ILLUSTRATION of its usual answer and not the test. A directed-differentiation cocktail of named bioactive factors used to obtain the cells is the model **even when an undifferentiated arm was sequenced alongside it** (v0.0.11) — but a paper comparing protocols or factor sets to determine what drives a fate is studying the cocktail, and that is a perturbation. v0.0.9 conditioned this rule on the formulation being "given uniformly to every sample", and that precondition inverted the call on `10.1016/j.stem.2022.11.013`, where LinPOS organoids in self-renewing medium and alveolar organoids in AT2 differentiation medium were both sequenced; the curator's ruling (2026-08-28) is that this is "a different target cell type, not perturbation... I don't really care what has been applied to get to the final diff cell type." **Usually — not always — the answer tracks identity versus factor: producing a different target cell IDENTITY is normally the model, and varying a named factor while the identity is held fixed is normally a perturbation. Treat that as a rule of thumb that the governing question overrides**, because it is a statement about the shape of the contrast and sharpening 2 above says the shape decides nothing. "fdAT2 organoids maintained in AT2 differentiation medium (dexamethasone, cAMP, IBMX, DAPT, FGF7, CHIR99021, A83-01, Y-27632)" is the model, and so is "LinPOS tip organoids differentiated for 7 days into alveolar organoids", because the endpoint is a new cell type. "The same alveolar organoids with FGF7 withdrawn versus control" is a perturbation, because the identity stays put and one named factor moves. The v0.0.6 carve-out is therefore intact and unchanged — it was always about a contrast in a factor, never about a contrast in cell identity. Do not list the formulation in `perturbations`; record it in `suppressed_candidates` with `rule: derivation_formulation`, because a reader who counted the cocktail would have found a perturbation here and the record must show that you did not. [TOGGLE: count directed differentiation as a perturbation if the curator wants any applied cocktail on profiled material flagged.]
...
- **"yes"** — the text explicitly links the perturbed sample/group to a qualifying single-cell/nucleus assay (e.g., "PBMCs were stimulated with LPS for 4h and then processed for 10x scRNA-seq"; "tumor-bearing mice received anti-PD-1 or vehicle and sorted CD45+ cells underwent snRNA-seq"). A Perturb-seq/CROP-seq-type screen is always "yes" by construction.
...
**"no" beats "unclear" once you can NAME the readout, and this is the distinction the three bullets above did not enforce (curator batch 2026-09-15).** "unclear" is for when you cannot tell which assay read the perturbation out. It is NOT for when you can tell, and the answer is a non-qualifying assay, but a qualifying single-cell dataset also exists somewhere else in the paper. That combination is exactly what the "no" bullet describes, and it is common: a paper runs its treatment arm on flow cytometry and its single-cell assay on separate unperturbed material.

The tell that you have already decided is your own reasoning. `10.1016/j.immuni.2020.03.019` was scored "unclear" while its stated grounds were that "every reported readout of it is flow cytometry and the scRNaseq populations were sorted separately" — which is the "no" bullet, verbatim in substance. If you can write that sentence, write `"no"`. Reserve `"unclear"` for a genuine gap in the text, not for residual unease about a call you have in fact made.
...
Every example above assumes a whole text and an empty `text_defects` — no count is given here on purpose, because the last one went stale. Add a verified defect in the main source, or a `no_methods_content` from anywhere, and examples 2, 5 and 7 are moved to "unclear" by Stage B's damaged text downgrade rather than resolving to "no". A defect in a supplementary source alone does not downgrade them: see Stage B.
...
- `reports_primary_research` = "yes" if the paper reports work its own authors did; "no" for a review, commentary, perspective, editorial or meeting report. See Step 0b — it is a question about article type, and a re-analysis or integration atlas is still "yes".
...
`perturbation_present` is the primary field driving curation. Compute it in two stages. Stage A reads only the pairing evidence; Stage B applies the damaged text downgrade. Run the consistency checks before Stage A.
...
**A-1.** `reports_primary_research` = "no" → **"not_applicable"**. The paper reports no study of its own, so there is nothing to pair. Numbered A-1 because it precedes the evidence rules, but evaluated AFTER A0 on purpose: a failed extraction cannot support a judgment about article type, so a paywall page is "unclear" and not "a review". A-1 is NOT folded into A1 — a gated paper's `perturbations` array is empty because Step 0b returned, not because no perturbation was found, and A1 would have read that emptiness as evidence.
...
Totality: A0 covers failed input; A-1 covers a non-primary article; A1 covers the empty array; for a non-empty array A2, A3, and A4-A6 partition the three values of `has_single_cell_assay`, and within `has_single_cell_assay` = "yes" the rules partition the pairing multiset into (contains a "yes") / (no "yes", contains an "unclear") / (all "no"). No input reaches the end unmatched.

### Stage B: damaged text downgrade
**This stage is applied by the harness, not by you**, and its trigger is two facts rather than a judgment. It fires when either:

1. the pipeline withheld text — the budget ladder ran, which `ASSEMBLY:` reports and the harness records itself; or
2. you reported a **verified** `text_defects` entry that is in scope: one in the **main** source, of any kind, or one of kind **`no_methods_content`** in any source.

Then:
- Stage A result **"no"** → **"unclear"**, with `unresolved_reason` = "degraded_text" and the reason stated in `ambiguities`.
- Stage A result **"yes"** or **"unclear"** → unchanged.
- Stage A result **"not_applicable"** → unchanged. The downgrade exists because missing text can hide a pairing sentence; a review has no pairing to hide, and a review very often reports `text_completeness` = "methods_missing" precisely because it has no Methods. Downgrading it would turn every review into an `unclear` for triage to re-read.

**The asymmetry is deliberate.** Missing text can hide the sentence that would have paired a perturbation to a single-cell assay, so a negative drawn from incomplete text is not trustworthy. It cannot invent a perturbation or a pairing, so a positive drawn from the text you do have stands on its own evidence.

**Why the scope is what it is.** The downgrade withholds a negative only where the missing text could have carried the pairing sentence. A garbled table in a supplementary reporting summary could not, so it does not downgrade — it is still recorded, and a curator still sees it. An absent methods section could, wherever it lives, so that one downgrades from any source. (Curator decision, 2026-09-16.)

**What this replaced, and why.** The trigger used to be `processing_status = "partial" OR text_completeness ≠ "full"` — your own summary of the text, with nothing behind it. It was not reproducible: 3 of 30 papers changed that answer between two byte-identical runs at v0.0.23, and 4 of 24 at v0.0.24 after the `ASSEMBLY:` block had removed the largest single cause. Those two fields are still on the record and still yours; they no longer decide anything. **What decides is a quote the harness can check.** [TOGGLE: to keep strict negatives on degraded text, disable Stage B and filter on `text_defects` downstream.]
...
| ok or partial, `reports_primary_research` = "no" | empty | any | any | n/a | not_applicable |
...
- **CC-8.** `reports_primary_research` = "no" with a non-empty `perturbations` array. Step 0b returns before Step 2, so this combination cannot arise from following the steps in order. Either the paper does report a study of its own — set the field to "yes" and continue normally — or those perturbations belong to the papers under review and are not this paper's to report. The harness computes CC-8 from the record too, so declare it in `consistency_flags` if you leave the contradiction in place.
...
```json
{
  ...
  "text_defects": [
    {"source_id": "string (a <<<SOURCE>>> id you were given)",
     "kind": "ends_mid_sentence | explicit_cut_marker | garbled_run | no_methods_content",
     "quote": "string, verbatim from that source; null ONLY for no_methods_content"}
  ],
  "reports_primary_research": "yes | no",
  "has_single_cell_assay": "yes | no | unclear",
  "single_cell_assay_types": ["e.g. '10x scRNA-seq', 'snATAC-seq', 'Perturb-seq'"],
  "perturbation_present": "yes | no | unclear | not_applicable",
  ...
  "suppressed_candidates": [
    {
      ...
      "rule": "reporter_or_marker | incidental_clinical_therapy | unintended_condition | derivation_formulation | disease_model_establishment | observational_disease_state | sample_handling_protocol | readout_reagent | routine_processing",
...
- `rule` is a **closed set**: exactly the values in the table under **Recording an exclusion**, and no others. An open string cannot be tallied, which is the whole point of the field. If a candidate seems to need a value that is not there, choose the closest and explain in `why` — the table has grown once, by curator ruling, and a gap in it is worth reporting rather than working around. No count of the set is stated anywhere on purpose: it was written in four places and would have gone stale in three of them the first time the set changed.
...
| A manipulation used to obtain the material the paper characterises — a germline disease genotype, an engraftment, an induction protocol whose resulting tissue is the subject | Not a perturbation; it is the model. Recorded in `suppressed_candidates`, `rule: disease_model_establishment` (v0.0.15, re-keyed on attribution at v0.0.16, curator rulings 2026-09-03) | Count disease models as perturbations |
...
| Degraded or truncated text that would otherwise resolve to "no" | Stage B's damaged text downgrade moves it to "unclear" with `unresolved_reason = degraded_text`, on a harness-verified defect in the main source or a missing methods section anywhere | Disable Stage B and keep the "no", filtering on `text_defects` downstream |
| A defect in a supplementary source only | Recorded in `text_defects`, does NOT downgrade — it could not have hidden a pairing sentence | Widen `decide.yaml: damaged_text_downgrade.when_defect_in_sources` to every source |
...
- **A paper that was never retrieved must never be silently absent from the output**, because absence is indistinguishable from a negative in any downstream count. The harness honours that by keeping it *visible as unscored* rather than by synthesizing a record: it stays in the manifest, `harness.pending` names it and why, and it appears in the summary with an open-string `status` and `triage_priority` 0. No model-free `processing_status = "failed"` record is written — a determination-shaped row for a paper nobody assessed is the thing this rule is against, not the way to satisfy it.
...
**Fill `ASSEMBLY:` from the assembly that just happened**, never from a description of the pipeline maintained beside it. `harness.prepare.assembly_note` renders it from the source list, the config's own exclusion and content-kind lists, and the truncation rung actually used, so it states: the sources supplied with their char counts, how many supplementary files were supplied of how many found, the sections removed from every source, the content kinds that reached the model, and which budget rung ran. Two runs over the same paper must render it byte-identically — Step 0's answer depends on it, and a fact that moves between runs is the defect this block was added to remove.

### 3. Truncation policy
If the assembled text exceeds the context budget, drop content in this fixed order and record the rung reached in the run's `truncation` block. **Do not set `text_completeness` here:** that field is the model's call in Step 0, on the text it was actually given, and having two owners would make it unreadable. What the harness owes the model instead is the FACT, stated in `ASSEMBLY:` where Step 0 can read it — v0.0.24, after two byte-identical runs disagreed about whether a paper was truncated whose pipeline cuts nobody had declared. A harness-applied truncation is still recorded beside the record by `harness.prepare.sources_within_budget`; `harness.validate` also writes it ONTO the record as `harness_withheld`, which is what Stage B's damaged text downgrade reads from v0.0.25 — a field of the harness's own rather than an overwrite of the model's, for the reason this paragraph gives. The one place the harness still overrides `text_completeness` is `harness.validate`, and only in the direction it can prove: it truncated, so "full" becomes "truncated" with `text_completeness_source = "harness"`. **`harness.prepare` additionally records `section_chars`**, the characters supplied under each section label, because a claim about an ABSENCE has no substring to verify and the only auditable question is how much labelled text arrived.
...
### 5. Parse
On JSON parse failure, retry once with a repair instruction ("return only the JSON object"). A second failure leaves the paper **unscored rather than scored as failed**: no record is written, `harness.pending` reports it as unparseable with the reason, and re-running step 2 picks up only those. There is no `error_code` field — step 9 says what is actually recorded.
...
2. `perturbation_present` is not `"yes"`, and some `suppressed_candidates[].would_have_paired = "yes"` **for a rule whose boundary is under review** — `reporter_or_marker`, `incidental_clinical_therapy`, `unintended_condition`, `derivation_formulation` (v0.0.10) and `disease_model_establishment` (**new in v0.0.15** — the newest boundary, and its entries almost always pair trivially, since an engraftment or an induced mutation pairs to the assay run on the tissue it produced). The remaining rules are long-settled toggles and are deliberately excluded: `observational_disease_state` pairs "yes" on any tumour-vs-normal or disease-vs-healthy contrast, so including it put 5 of the 6 regression papers in this tier, and a tier holding most papers is not a queue. Both the restricted and unrestricted counts are reported (`suppressed_would_pair_yes_under_review` and `suppressed_would_pair_yes`), so the share of the suppression load coming from settled toggles stays visible. One toggle flips these papers: the candidate is named, its pairing is already judged, and the only thing standing between the paper and a `"yes"` is a NOT-list rule. Priority 1 stays above it because that bucket is an open question a reader must resolve; this bucket is a settled call a curator must ratify — narrower, but far more actionable.
3. *(vacant from 0.0.22.)* This slot held `perturbation_present = "yes"` with `paper_confidence < 0.6` — the only rule anywhere in this pack that read the confidence number. Across the nine paired same-input re-runs, **11 of the 16 papers that entered this tier in either run flipped in or out of it**, so its membership was largely an artefact of which pass you ran. The slot is left empty rather than reused or closed up: renumbering is the documented trap below, and it has already cost one version boundary.
