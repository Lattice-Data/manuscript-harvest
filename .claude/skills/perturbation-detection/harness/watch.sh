...
#
#   ./harness/watch.sh <work_dir>          # print progress every 30s, then notify
#   ./harness/watch.sh <work_dir> status   # print progress once and exit
#
...
WORK="${1:?usage: ./harness/watch.sh <work_dir> [status]}"
MODE="${2:-watch}"
...
progress() {
  # "done" means what harness.pending means by it -- parses, carries every required
...
# was never written -- `./harness/watch.sh work status` used to print
# "0/ done, 0 failed  FINISHED" for a work dir that did not exist.
if [ ! -f "$WORK/manifest.json" ]; then
  echo "no run at $WORK -- no manifest.json. Run harness.prepare first, or check the path." >&2
  exit 2
fi
...
echo
echo "next:"
echo "  $PY -m harness.pending   --work $WORK          # anything to re-run?"
