...
| `disease_model_establishment` | the manipulation whose purpose is to ESTABLISH the disease state the paper studies — engrafting tumour cells or organoids into a host, activating an allele to produce the lesion, a chemical or surgical induction protocol, a transgenic disease model | **No** — not by the paper's interest in it, and not by comparing two ways of making the model. What promotes is something applied to the ESTABLISHED model at fixed disease state |
...
- **A manipulation whose purpose is to ESTABLISH the disease state under study is the MODEL, not a perturbation of it.** Engrafting tumour cells or organoids into a host, activating an oncogenic or tumour-suppressor allele to produce the lesion, a chemical or surgical induction protocol, a transgenic disease model: the applied thing is how the sample came to exist, and the sequenced contrast is diseased tissue against healthy — a STATE contrast. Do not list it in `perturbations`; record it in `suppressed_candidates` with `rule: disease_model_establishment`. (Curator rulings of 2026-09-03 on `10.1016/j.ccell.2025.12.003` and `10.1016/j.cell.2021.11.031`.)
  - **The route does not matter.** One of those papers engrafted CRISPR-engineered AKPS organoids; the other injected 4-hydroxytamoxifen through the colonoscope to activate Cre in resident cells, against a PBS-injected control. Both are the model. A rule keyed on "cells were introduced" would have caught the first and missed the second.
...
| A manipulation whose purpose is to establish the disease state under study (engraftment, an allele activated to produce the lesion, a chemical or surgical induction protocol) | Not a perturbation; it is the model. Recorded in `suppressed_candidates`, `rule: disease_model_establishment` (v0.0.15, curator rulings 2026-09-03) | Count engineered and induced disease models as perturbations |
