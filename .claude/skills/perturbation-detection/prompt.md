...
- `would_have_paired` is the pairing this candidate WOULD have received under Step 3 had it been listed — the same yes/no/unclear judgment, held to the **same evidence standard**, not a marker for how notable the candidate feels. If the text does not tie the candidate to the material that went into the single-cell assay, the answer is "unclear" or "no". A record whose every suppressed candidate is "yes" has stopped discriminating, and the column is worthless the moment it becomes automatic. It is the highest-value column in the record: a suppressed candidate that would have paired "yes" sits one toggle away from flipping the paper to "yes", so those rows triage near the top (step 10, priority 2).
