...
- `would_have_paired` is the pairing this candidate WOULD have received under Step 3 had it been listed — the same yes/no/unclear judgment on the same evidence. It is the highest-value column in the record: a suppressed candidate that would have paired "yes" sits one toggle away from flipping the paper to "yes", so those rows triage near the top (step 10, priority 2).
