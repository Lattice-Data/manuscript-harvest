...
**Recording an exclusion from this list.** When one of these items was a genuine candidate — the paper puts it in a perturbing role and you decided it was not one — record it in `suppressed_candidates` rather than leaving the call invisible. Use `rule: readout_reagent` for detection and staining antibodies, dyes, a reporter read out as a proxy for expression, and delivery reagents carrying no functional cargo; `rule: routine_processing` for lysis and extraction reagents, wash buffers, fixation and permeabilization, blocking reagents, standard media and sera, sterility antibiotics, standard incubation and centrifugation, cryopreservation and dissociation, and library/sequencing chemistry. The three bolded bullets above — reporter transgenes, selection markers, epitope tags — have their own value, `reporter_or_marker`; use that in preference to `readout_reagent`. The threshold is a call you actually had to make: do not enumerate every reagent the Methods happen to name. **These are NOT suppressed candidates in an ordinary paper** — a ROCK inhibitor (Y-27632) in a dissociation or wash buffer, a preservation solution or a 4 °C hold before processing (HypoThermosol, cold ischaemic time), DMSO used only for freezing, a dissociation enzyme, pen/strep, a library-prep kit. They become entries **only** in a paper that makes them its studied variable — a media-brand comparison, a storage-duration series, a dissociation-enzyme benchmark. In every other paper they are simply not candidates, and listing them buries the one suppression that mattered.
...
- **A condition the paper describes as unintended, accidental, or inferred after the fact is NOT a perturbation**, even when it demonstrably affected the profiled samples and therefore pairs trivially to the assay. The test is whether the investigators APPLIED it as a condition. Hedged, retrospective language is the tell: "we now understand they likely became", "may have been", "appears to have inadvertently". Such a condition has no arm to compare against, and typically no dose, duration or level on record. Do not list it in `perturbations`; record it in `suppressed_candidates` with `rule: unintended_condition` — it is real information about sample quality even though it is not an experimental variable, and it typically pairs trivially, so `would_have_paired` will often be "yes".
  - **Precedence against the clinical-therapy rule: what must be hedged is the condition's own EXISTENCE or APPLICATION, not its effect.** "We now understand the cultures likely became somewhat hypoxic" hedges whether the condition happened at all — that is an unintended condition and stays suppressed. A patient who received chemotherapy between a sequenced diagnosis sample and a sequenced relapse sample was *definitely* treated, and treated deliberately; what the paper infers is the causal link to the phenotype it then observes. **An inferred CAUSE does not make an applied condition unintended.** Where two sequenced timepoints straddle the therapy, that is a per-sample tie and the clinical-therapy report rule wins: report it as a chemical perturbation, do not suppress it. The disambiguating question is whether you could say who applied the condition and when — if you can, it was applied, however hedged the paper is about what it did. [TOGGLE: count inadvertent conditions if the curator wants any non-baseline state of the profiled cells flagged.]
- **The defining derivation or maintenance formulation of a model system is NOT a perturbation**, even when it is a directed-differentiation cocktail of named bioactive factors and even when the paper says it drives a fate change. The v0.0.6 carve-out below — a named bioactive component delivered through the medium still counts — is about a CONTRAST: adding, withholding, substituting or dose-varying that factor against a comparator. When every sample in the experiment saw the same formulation and there is no contrast arm, the formulation is the definition of the model, not a variable applied to it. "fdAT2 organoids maintained in AT2 differentiation medium (dexamethasone, cAMP, IBMX, DAPT, FGF7, CHIR99021, A83-01, Y-27632)" is the model; "the same organoids with FGF7 withdrawn versus control" is a perturbation. Do not list the formulation in `perturbations`; record it in `suppressed_candidates` with `rule: derivation_formulation`, because a reader who counted the cocktail would have found a perturbation here and the record must show that you did not.
...
| A model's defining derivation / directed-differentiation formulation, applied uniformly with no contrast arm | Not a perturbation; only add/withhold/substitute/dose-vary contrasts count (v0.0.9) | Count directed differentiation as a perturbation |
...
| **Papers with a suppressed candidate that would have paired `yes`** (v0.0.10) | The review queue for the boundary rules: every one of these is a paper the rules alone kept out of `yes` |
